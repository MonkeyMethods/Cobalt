...
function setBackend(backend) {
	...
	switch (backend) {
		case "raknet-node":
			...
			return {
				RakServer: RakNativeServer,
				RakClient: RakNativeClient,
				RakTimeout,
			};
		case "raknet-native":
			({
...
module.exports = (backend) => {
	...
	try {
		...
		console.debug(
			`[raknet] ${backend} library not found, defaulting to jsp-raknet. Correct the "raknetBackend" option to avoid this error.`,
			e,
		);
...
class RakNativeClient extends EventEmitter {
	constructor(options, client) {
		...
		const protocolVersion = client?.versionGreaterThanOrEqualTo("1.19.30")
			? 11
			: 10;
		this.raknet = new Client(options.host, options.port, { protocolVersion });
		this.raknet.on("encapsulated", ({ buffer, address }) => {
			if (this.connected) {
				// Discard packets that are queued to be sent to us after close
		...
		return waitFor(
			...
			() => {
				if ("REPLIT_ENVIRONMENT" in process.env) {
					console.warn(
						"A Replit environment was detected. Replit may not support the necessary outbound UDP connections required to connect to a Minecraft server. Please see https://github.com/PrismarineJS/bedrock-protocol/blob/master/docs/FAQ.md for more information.",
					);
				}
	...
	sendReliable(buffer, immediate) {
		...
		const priority = immediate
			? PacketPriority.IMMEDIATE_PRIORITY
			: PacketPriority.MEDIUM_PRIORITY;
		return this.raknet.send(
			buffer,
			priority,
...
class RakJsClient extends EventEmitter {
	constructor(options = {}) {
		...
		if (options.useWorkers) {
			...
			this.close = (reason) =>
				this.worker?.postMessage({ type: "close", reason });
	...
	async plainConnect(host = this.options.host, port = this.options.port) {
		...
		this.raknet.on("encapsulated", (encapsulated, addr) =>
			this.onEncapsulated(encapsulated, addr.hash),
		);
...
class RakJsServer extends EventEmitter {
	constructor(options = {}, server) {
		...
		this.onOpenConnection = () => {};
		this.onCloseConnection = () => {};
		this.onEncapsulated = (packet, address) =>
			server.onEncapsulated(packet.buffer, address);
		this.onClose = () => {};
		this.updateAdvertisement = () => {
